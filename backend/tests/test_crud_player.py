...
class TestPlayerSearch:
    ...
    @pytest.mark.search
    def test_search_all_fields_name_match(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    def test_no_filters_provided(
        ...
        assert expected_names.issubset(
            player_names
        ), "Missing expected players in results"

    @pytest.mark.search
    def test_search_all_fields_team_match(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.search
    def test_search_specific_field_name(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.search
    def test_search_team_field_direct(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.search
    def test_search_position_field(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.search
    def test_search_jersey_number_invalid(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    def test_search_nationality(
        ...
        assert total >= 1
        for player in players:
            nationality_value = player.nationality.lower()
...
class TestPlayerFiltering:
    ...
    @pytest.mark.filter
    def test_filter_position_equals(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    def test_filter_position_not_equals(
        ...
        assert total >= 1
        for player in players:
            assert player.position != "Goalie"  # type: ignore

    @pytest.mark.filter
    def test_filter_position_contains(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.filter
    def test_filter_position_not_contains(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
        ...
        """
        filters = [
            PlayerFilter(field="position", operator="not_contains", value="Wing")
        ]
        search_params = PlayerSearchParams(
            search=None,
            field="all",
    ...
    @pytest.mark.filter
    def test_filter_goals_greater_than(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
        ...
        for player in players:
            assert player.goals > 20  # type: ignore

    @pytest.mark.filter
    def test_filter_goals_less_than_equal(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
        ...
        for player in players:
            assert player.goals <= 10  # type: ignore

    @pytest.mark.filter
    @pytest.mark.parametrize(
        "operator,test_value,expected_condition",
        [
    ...
    def test_filter_goals_numeric_operators(
        self,
        test_db: Session,
        specific_test_players: list[Player],
        operator: str,
        test_value: int,
        expected_condition,
    ):
        ...
        filters = [
            PlayerFilter(
                field="goals",
                operator=cast(FilterOperatorType, operator),
                value=test_value,
            )
        ]
        search_params = PlayerSearchParams(
        ...
        for player in players:
            assert expected_condition(
                player.goals
            ), f"Player {player.name} goals {player.goals} failed condition for {operator} {test_value}"

    @pytest.mark.filter
    def test_filter_active_status_true(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.filter
    def test_filter_active_status_not_equals(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.filter
    def test_filter_active_status_string_conversion(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.filter
    def test_filter_multiple_and_logic(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
...
class TestPlayerSorting:
    ...
    @pytest.mark.sort
    def test_sort_by_goals_descending(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.sort
    def test_sort_by_active_status_ascending(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
        ...
        for i, player in enumerate(players):
            if not player.active_status:  # type: ignore
    ...
    @pytest.mark.sort
    def test_sort_by_active_status_descending(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
        ...
        for i, player in enumerate(players):
            if player.active_status:  # type: ignore
    ...
    def test_sort_by_team_name(
        ...
        if len(players) > 1:
            for i in range(len(players) - 1):
                current_team = players[i].team.name
                next_team = players[i + 1].team.name
...
class TestPagination:
    ...
    def test_pagination_first_page(
        self, test_db: Session, sample_players: list[Player]
    ):
        """
    ...
    def test_pagination_with_search(
        ...
        if len(players) > 0:
            for player in players:
                name_match = "test" in player.name.lower()
                team_match = "test" in player.team.name.lower()
                assert name_match or team_match

    @pytest.mark.pagination
    def test_get_all_players_paginated(
        self, test_db: Session, sample_players: list[Player]
    ):
        """
...
class TestCombinedOperations:
    ...
    def test_search_filter_sort_combined(
        self, test_db: Session, sample_players: list[Player]
    ):
        """
        ...
        for player in players:
            ...
            search_match = (
                "test" in player.name.lower() or "test" in player.team.name.lower()
            )
    ...
    @pytest.mark.integration
    def test_empty_results_combined(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
...
class TestEdgeCases:
    ...
    @pytest.mark.edge_case
    def test_empty_search_with_filters(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
    ...
    @pytest.mark.edge_case
    def test_whitespace_only_search(
        self, test_db: Session, specific_test_players: list[Player]
    ):
        """
