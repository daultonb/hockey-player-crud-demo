...
def assert_player_comparison(
    player1: Player, player2: Player, field: str, comparison: str
) -> None:
    """
...
def assert_contains_substring(obj: Any, field_path: str, substring: str) -> None:
    ...
    assert (
        substring in field_value
    ), f"'{substring}' not found in {field_path}: '{field_value}'"


def assert_players_sorted(
    players: list[Player], sort_field: str, descending: bool = False
) -> None:
    """
    ...
    for i in range(len(players) - 1):
        ...
        if descending:
            ...
            ), f"Sort order violation at index {i}: {current_value} < {next_value}"
        else:
            assert (
                current_value <= next_value
            ), f"Sort order violation at index {i}: {current_value} > {next_value}"
